...
multfin<-import(here("data","data_covid19vaccine_menstrualcycles_uk_names.Rda")) # imputed list

# original data used for imputation
dataor<-import(here("data","data_covid19vaccine_menstrualcycles_uk_original.Rda")) # original dataset of vaccinated people
...
## add variable lifesat changes describes increases in dissatifaction
multfin<-lapply(mult,function(x) mutate(x,lifesatchanges=(lifesat_lastyr-lifesat_before) ))


## drops levels
multfin<-lapply(multfin,function(x) droplevels(replace(x, x=="", NA)))

## recode
multfin<-lapply(multfin,function(x) mutate(x, contra_current_groupall = recode_factor(contra_current_groupall,HormonalP ="Progestin only",HormonalPE="Combined")))

multfin<-lapply(multfin,function(x) mutate(x, vaccine_changescycle = recode_factor(vaccine_changescycle, "Other (please state)" ="Other","Yes, my menstrual cycles are LESS disrupted"="LESS disrupted", "Yes, my menstrual cycles are MORE disrupted"="MORE disrupted","No"="No changes")))



# Find which ones have IUD
# there are some who say Progestin and IUD -> we exclude them
table(dataor$contra_current_groupall[dataor$contra_current_coppercoil_num=="1"])


multfin<-lapply(multfin,function(x) 
    mutate (x,
            contra_current_groupall = case_when(  
                contra_current_groupall=="Progestin only" & contra_current_coppercoil_num== 0 ~ "Progestin only",
                contra_current_groupall=="Combined" & contra_current_coppercoil_num== 0 ~ "Combined",
                contra_current_groupall=="Non Hormonal" & contra_current_coppercoil_num== 0 ~ "Other",
                contra_current_groupall=="None" & contra_current_coppercoil_num== 0 ~ "None",
                contra_current_groupall=="Other" & contra_current_coppercoil_num== 0 ~ "Other",
                contra_current_coppercoil_num== 1 ~ "Copper IUD",
            )))
...
for (i in (1:5)){
    levels(multfin[[i]]$covid_tested)[2]<-"Self diagnosed +"
}


############################### recode for binary response
for (i in 1:length(multfin)){ 
    multfin[[i]]$vaccine_changescycle_binary<-fct_collapse(multfin[[i]]$vaccine_changescycle, Other = c("Other","LESS disrupted","MORE disrupted"))
}

## recode
multfin<-lapply(multfin,function(x) mutate(x, vaccine_changescycle_binary = recode_factor(vaccine_changescycle_binary,"Other" ="1", "No changes"="0")))

multfin<-lapply(multfin,function(x) 
    mutate(x, 
           vaccine_changescycle_binary =as.numeric(as.character(vaccine_changescycle_binary)
           )
    )
)
...
# exposures <- colnames(data)[which(colnames(data) != "participant_ID")]

# # or if you have a few columns to exclude, do it this way
# exposures <- colnames(data)[which(colnames(data) %nin% c("participant_ID",
#                                                          "old_outcome",
#                                                          "unwanted_var"))]
...
train.rows <- createDataPartition(y = recoded_data[[1]]$vaccine_changescycle_binary, # create partition with same proportion of outcome variable
...
# make model matrix of exposures
...
auc.lasso <-  auc(recoded_data_rep$outcome, pred)
```
...
coefs <- coef(cv.fit.lasso, 
              newdata = x_rep, 
              s = final_lambda, # optimized lambda value
              alpha = cv.fit.lasso$lambda.1se, # optimized lambda value. You can also try "cv.fit.lasso$lambda.min"
              exact = FALSE)
...
# set vector of alpha values to test between 0-1
alpha_grid <- seq(0, 1, 0.1)

# create list for all model fits
cv.fit.enet <- as.list(seq(alpha_grid))

# run elastic net (loops the glmnet model and each time tries a new value for alpha using the alpha_grid)
for (k in seq(cv.fit)) {
    set.seed(3456)
    cv.fit.enet[[k]] <- 
        cv.glmnet(x = x,
                  ...
                  nfolds = 10, # 10-fold CV
                  foldid = foldid,
                  trace.it = 1, T# show progress bar
                  alpha = alpha_grid[[k]] # elastic net
        )
}
