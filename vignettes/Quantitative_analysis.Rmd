...
tbl<- 
  tbl_summary(
  select(dataor , age, edu_group,liveuk,ethnic_group,income_before, smoke_before,maritalstatus, maritalstatus, gender, nbdeliveries,contra_current_groupall, covid_group,covid_tested, vaccinated, vaccine_type,vaccine_timing),
 label = list(age ~ "Age",
            edu_group~ "Education level",
            liveuk~ "Place of residence",
            ethnic_group~ "Ethnic group",
            income_before~ "Net income before pandemic", 
            smoke_before~ "Smoking status before pandemic",
            maritalstatus~ "Marital status", 
            gender~ "Gender", 
            nbdeliveries~ "Number of deliveries",
            contra_current_groupall~ "Contraceptive use at the time of the survey", 
            covid_group~ "Self-reported COVID status",
            covid_tested~ "COVID status based on testing", 
...
### AUC
Now we investigate the discriminative utility of each model by computing the AUC and drawing AUC-ROC curves.
```{r auc_univariable}
library(pROC)
...
The plot displays for each variable, the AUC of the model with each of the 5 imputed datasets. Overall, AUCs are low and the univariable models have poor discriminative utility.

## Multivariable analyses
### Models and output tables
We take each exposure significant in the univariable models at q.value (e.g., FDR adjusted p.value) < 0.05 (Smoking, Menstrual cycles changes, COVID history and menstrual cycle changes over the last year). Based on DAGS (SI5), only COVID status and menstrual cycle changes should be adjusted for confounders.


#### Models 3 & 4. COVID status as exposure
...
all_models3$term <- c("Past smoker","Current smoker","Combined","Copper IUD","Other","Progestin only","Sterilized","Self diagnosed +","Tested +")
...
#### Models 5 & 6  Menstrual cycle changes over the last year as exposure

```{r logistic_multivariate_ms}
# set global R options
pboptions(type = "timer", char = "=") # initialize progress bar
...
For the full model, AUCs vary between 0.57 and 0.61 depending on which imputed dataset is used.
...
ggsave(filename="Figure3.pdf", width = 30,  height = 20, units = "cm")
...
d<-plot(Effect("changesmenses_num",modelchangemense), 
     lines = list(multiline = T), 
     lty=0,
     lwd=6,
     ylim=c(-3,-0.3),
     ...
     confint = list(style = "auto"),
     response="",
     xlab="Pandemic-related menstrual changes", 
     ylab= "Predicted probability",
     main="")
...
# save
pdf(file='Figure2.pdf', width=15,height=8) # Open PDF device with specific file name
...
levels(dataor$smoke_before)[3]<-"Current Smoker"
levels(dataor$smoke_before)[4]<-"Current Smoker"
dataor$contra_current_groupall<-as.factor(dataor$contra_current_groupall)
dataor$contra_current_groupall<-relevel(dataor$contra_current_groupall, ref = "None")


## models--------------------------------------------------------
model1<-glm(vaccine_changescycle_binary ~ smoke_before, data =dataor, family=binomial, model = TRUE)

## model 2----------------------------------------------------

model2<-glm(vaccine_changescycle_binary ~ contra_current_groupall, data =dataor, family=binomial, model = TRUE)

## model 3----------------------------------------------------

model3<-glm(vaccine_changescycle_binary ~   smoke_before+contra_current_groupall+covid_tested, data =dataor, family=binomial, model = TRUE)

## model 4----------------------------------------------------

model4<-glm(vaccine_changescycle_binary ~    smoke_before+contra_current_groupall+covid_group, data =dataor, family=binomial, model = TRUE)

## model 5----------------------------------------------------

model5<-glm(vaccine_changescycle_binary ~    smoke_before+contra_current_groupall+ changesmenses_num+covid_tested, data =dataor, family=binomial, model = TRUE)

## model 5----------------------------------------------------

model6<-glm(vaccine_changescycle_binary ~   smoke_before+contra_current_groupall+ changesmenses_num+covid_group, data =dataor, family=binomial, model = TRUE)
